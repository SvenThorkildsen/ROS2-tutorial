...
class Node1(Node):
    def __init__(self):
        ...
        self.cb_group2_ = MutuallyExclusiveCallbackGroup() # Enables parallel processing -> every callback function runs simultaneously with a period of 2 sec between each time
...
class Node2(Node):
    def __init__(self):
        ...
        self.cb_group1_ = ReentrantCallbackGroup() # Works like a single-threaded executor

        self.timer4_ = self.create_timer(1.0, self.callback_timer4, callback_group=self.cb_group1_)
        self.timer5_ = self.create_timer(1.0, self.callback_timer5, callback_group=self.cb_group1_)
